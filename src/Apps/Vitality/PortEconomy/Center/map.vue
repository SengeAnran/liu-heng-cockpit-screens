    ...
    getData() {
      getPortDetail()
        ...
        .then((json) => {
          ...
          const topIndex = json.findIndex((item, index) => {
           return item.name === '上海港 吴淞码头';
          });
          const topXytIndex = json.findIndex((item, index) => {
           return item.name === '美国西雅图港';
          });
          const topAmIndex = json.findIndex((item, index) => {
           return item.name === '美国长滩港';
          });
          const topLiuIndex = json.findIndex((item, index) => {
           return item.name === '宁波舟山港 六横港区';
          });
          console.log(topIndex);
          json[topIndex].top = '-8rem';
          json[topXytIndex].top = '-8rem';
          json[topAmIndex].top = '12rem';
          json[topLiuIndex].top = '2rem';
          json[topLiuIndex].left = '1rem';
          this.geoCoordMap = json;
        ...
        const marker = new AMap.Marker({
          ...
          content: `
            <div class="custom-marker">
              <div class="name" style="top: ${item.top};left: ${item.left};">${item.name}</div>
            </div>`,
        });
